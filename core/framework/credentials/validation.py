...
def ensure_credential_key_env() -> None:
    ...
    from .key_storage import load_oauth_host_api_key, load_credential_key
    ...
    try:
        ...
        for env_path in [_root / ".env", Path.cwd() / ".env"]:
            if env_path.exists():
    ...
    # Step 1: ENGINE_CREDENTIAL_KEY (must come first — encrypted store depends on it)
    load_credential_key()

    # Step 2: ENGINE_OAUTH_API_KEY (uses encrypted store, then shell config fallback)
    load_oauth_host_api_key()

    # Step 3: Load remaining LLM/tool API keys from shell config
    try:
        from engine_tools.credentials.shell_config import check_env_var_in_shell_config
    except ImportError:
        return
    ...
    try:
        ...
        for spec in CREDENTIAL_SPECS.values():
            ...
            if var_name and var_name not in ("ENGINE_CREDENTIAL_KEY", "ENGINE_OAUTH_API_KEY"):
...
class CredentialStatus:
    ...
    engine_oauth_supported: bool
    direct_api_key_supported: bool
    credential_key: str
    oauth_not_connected: bool  # Engine-only cred, ENGINE_OAUTH_API_KEY set, but integration missing
...
class CredentialValidationResult:
    ...
    has_oauth_host_key: bool

    @property
    def failed(self) -> list[CredentialStatus]:
        """Credentials that are missing, invalid, or Engine-not-connected"""
    ...
    def failed_cred_names(self) -> list[str]:
        """Credential names that need"""
        return [c.credential_name for c in self.failed if not c.oauth_not_connected]

    def format_error_message(self) -> str:
        """Format a human-readable error message for CLI/runner output"""
        missing = [c for c in self.credentials if not c.available and not c.oauth_not_connected]
        invalid = [c for c in self.credentials if c.available and c.valid is False]
        oauth_nc = [c for c in self.credentials if c.oauth_not_connected]
        ...
        if oauth_nc:
            if missing or invalid:
                lines.append("")
            lines.append(
                "Engine integrations not connected "
                "(ENGINE_OAUTH_API_KEY is set but OAuth tokens unavailable):\n"
            )
            for c in oauth_nc:
                lines.append(
                    f"  {c.env_var} for {_label(c)}"
                    f"\n    Connect this integration at engine.localhost first."
                )
...
def _presync_oauth_host_tokens(credential_specs: dict, *, force: bool = False) -> None:
    """Sync Engine-backed OAuth tokens into env vars for validation"""
    from framework.credentials.store import CredentialStore

    try:
        oauth_store = CredentialStore.with_engine_sync(auto_sync=True)
    except Exception as e:
        logger.warning("Engine pre-sync unavailable: %s", e)
        return

    for name, spec in credential_specs.items():
        if not spec.engine_oauth_supported:
            continue
        if not force and os.environ.get(spec.env_var):
            continue  # Already set — don't overwrite
        cred_id = spec.credential_id or name
        # sync_all() already fetched everything available from Engine.
        # Skip credentials not in the store — they aren't connected,
        # so fetching individually would fail with "Invalid integration ID".
        if not oauth_store.exists(cred_id):
            continue
        try:
            value = oauth_store.get_key(cred_id, spec.credential_key)
            if value:
                os.environ[spec.env_var] = value
                logger.debug("Pre-synced %s from Engine", spec.env_var)
            else:
                logger.warning(
...
def validate_agent_credentials(
    ...
    """Check that required credentials are available and valid before"""
    empty_result = CredentialValidationResult(credentials=[], has_oauth_host_key=False)
    ...
    try:
        ...
        return empty_result  # engine_tools not installed, skip check

    from framework.credentials.store import CredentialStore

    # Pre-sync: when ENGINE_OAUTH_API_KEY is available, sync OAuth tokens from Engine
    if os.environ.get("ENGINE_OAUTH_API_KEY"):
        _presync_oauth_host_tokens(CREDENTIAL_SPECS, force=force_refresh)

    # Build store: always check env vars first (.env, exports), then fallbacks.
    ...
    if credential_store is not None:
        # Layer env (primary) over app's store — validation sees BOTH
        # env vars/.env AND credentials saved via the dashboard modal.
        storage = CompositeStorage(
            primary=env_storage,
            fallbacks=[credential_store._storage],
        )
        store = CredentialStore(storage=storage)
    elif os.environ.get("ENGINE_CREDENTIAL_KEY"):
        storage = CompositeStorage(primary=env_storage, fallbacks=[EncryptedFileStorage()])
        store = CredentialStore(storage=storage)
    else:
        store = CredentialStore(storage=env_storage)
    ...
    has_oauth_host_key = bool(os.environ.get("ENGINE_OAUTH_API_KEY"))
    checked: set[str] = set()
    ...
    def _check_credential(
        ...
        # Engine-not-connected: ENGINE_OAUTH_API_KEY set, Engine-only cred, but integration missing
        is_oauth_host_nc = (
            not available
            and has_oauth_host_key
            and spec.engine_oauth_supported
        ...
        status = CredentialStatus(
            ...
            oauth_not_connected=is_oauth_host_nc,
    ...
    if to_verify:
        try:
            from engine_tools.credentials import check_credential_health
        except ImportError:
    ...
    validation_result = CredentialValidationResult(
        ...
        has_oauth_host_key=has_oauth_host_key,
    )
...
def _status_to_missing(c: CredentialStatus):
    ...
    return MissingCredential(
        ...
        engine_oauth_supported=c.engine_oauth_supported,
