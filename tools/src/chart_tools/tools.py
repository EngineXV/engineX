...
def _system_theme() -> str:
    ...
    The Electron main process sets ``ENGINE_DESKTOP_THEME`` to ``"light"``
    ...
    """
    val = (os.environ.get("ENGINE_DESKTOP_THEME") or "").strip().lower()
...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def chart_render(
        ...
        Args:
            ...
            output_path: Override where the PNG lands. Default is
                  $ENGINE_HOME/charts/<UTC-timestamp>-<title>.png.

        Note: theme is NOT an argument. It's resolved automatically
        from the desktop's ENGINE_DESKTOP_THEME env var so the saved
        ...
        # Electron main process sets ENGINE_DESKTOP_THEME from
    ...
    # $ENGINE_HOME/charts/ with the same outcome.
...
def _engine_home() -> Path:
    override = os.environ.get("ENGINE_HOME")
    if override:
