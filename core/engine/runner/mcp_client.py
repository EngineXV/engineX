...
class MCPClient:
    ...
    def _connect_stdio(self) -> None:
        ...
        try:
            ...
            # The connection task must stay alive so cleanup runs in the SAME task
            # that entered the context managers (fixes AnyIO cancel-scope error).
            ...
            def run_event_loop():
                ...
                shutdown_requested = self._loop.create_future()  # Created in loop thread
                self._shutdown_future = shutdown_requested
                loop_started.set()

                async def connection_lifecycle():
                    """Enter contexts, wait for shutdown, then exit in same task"""
                    try:
                        ...
                        # Wait for shutdown signal (keeps this task alive for same-task cleanup)
                        await asyncio.shield(shutdown_requested)
                        ...
                        # Cleanup in SAME task that entered contexts
                        try:
                            if self._session:
                ...
                self._loop.create_task(connection_lifecycle())
    ...
    def disconnect(self) -> None:
        ...
        # Clean up persistent STDIO connection by signalling the connection task
        # to exit (cleanup runs in same task, fixing AnyIO cancel-scope error)
        if self._loop is not None:
            ...
            if self._loop.is_running():
                try:
                    ...
                    if shutdown_future is not None and not shutdown_future.done():
                        ...
                        self._loop.call_soon_threadsafe(signal_shutdown)
                        shutdown_future.result(timeout=self._CLEANUP_TIMEOUT)
                    cleanup_attempted = True
                except TimeoutError:
                    cleanup_attempted = True
                    logger.warning(f"MCP disconnect timed out after {self._CLEANUP_TIMEOUT}s")
                except Exception as e:
                    cleanup_attempted = True
                    logger.debug(f"Error signalling MCP shutdown: {e}")
