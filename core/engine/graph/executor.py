...
from engine.graph.output_cleaner import CleansingConfig, OutputCleaner
from engine.graph.validator import OutputValidator
from engine.llm.provider import LLMProvider, Tool
from engine.observability import set_trace_context
...
class GraphExecutor:
    ...
    async def _phase_llm_compact(
        ...
        from engine.graph.event_loop_node import _is_context_too_large_error
    ...
    async def execute(
        ...
        if tool_errors:
            self.logger.error("ERROR: Tool validation failed:")
        ...
        if checkpoint_config and checkpoint_config.enabled and self._storage_path:
            ...
            self.logger.info("OK: Checkpointing enabled")
        ...
        if session_state and "memory" in session_state:
            ...
            if not isinstance(memory_data, dict):
                self.logger.warning(
                    f"WARN: Invalid memory data type in session state: "
        ...
        if session_state and session_state.get("resume_from_checkpoint") and checkpoint_store:
            ...
            if paused_at and graph.get_node(paused_at) is not None:
                ...
                self.logger.warning(
                    f"WARN: paused_at={paused_at} is not a valid node, falling back to entry point"
                )
                current_node_id = graph.get_entry_point(session_state)
        ...
        try:
            while steps < graph.max_steps:
                ...
                if _is_resuming and path:
                    from engine.graph.prompt_composer import build_narrative
                ...
                if validation_errors:
                    self.logger.warning(f"WARN: Validation warnings: {validation_errors}")
                ...
                if result.success:
                    ...
                    if (
                        ...
                        if not validation.success:
                            self.logger.error(
                                f"   FAIL: Output validation failed: {validation.error}"
                            )
                            result = NodeResult(
                                success=False,
                ...
                if node_spec.id in graph.terminal_nodes:
                    self.logger.info(f"OK: Reached terminal node: {node_spec.name}")
                    break
    ...
    def _build_context(
        ...
        if self.accounts_data and self.tool_provider_map:
            from engine.graph.prompt_composer import build_accounts_prompt
    ...
    def _get_node_implementation(
        ...
        if node_spec.node_type in self.REMOVED_NODE_TYPES:
            ...
            raise RuntimeError(
                ...
                f"See https://github.com/mishrapravin114/engine/issues/4753 for migration guide."
            )
        ...
        if node_spec.node_type == "event_loop":
            ...
            from engine.graph.event_loop_node import EventLoopNode, LoopConfig
            ...
            if await edge.should_traverse(
                ...
                if self.cleansing_config.enabled and target_node_spec:
                    ...
                    if not validation.valid:
                        self.logger.warning(f"WARN: Output validation failed: {validation.errors}")
                        ...
                        if revalidation.valid:
                            self.logger.info("OK: Output cleaned and validated successfully")
                        else:
                            self.logger.error(
                                f"FAIL: Cleaning failed, errors remain: {revalidation.errors}"
                            )
    ...
    async def _execute_parallel_branches(
        ...
        async def execute_single_branch(
            ...
            try:
                ...
                if self.cleansing_config.enabled and node_spec:
                    ...
                    if not validation.valid:
                        self.logger.warning(
                            f"WARN: Output validation failed for branch "
                ...
                for attempt in range(effective_max_retries):
                    ...
                    if result.success:
                        ...
                        self.logger.info(
                            f"      OK: Branch {node_spec.name}: success "
                ...
                branch.status = "failed"
                branch.error = str(e)
                self.logger.error(f"      FAIL: Branch {branch.node_id}: exception - {e}")
        ...
        if failed_branches:
            ...
            if self._parallel_config.on_branch_failure == "fail_all":
                ...
                self.logger.warning(
                    f"WARN: Some branches failed ({failed_names}), continuing with successful ones"
                )
