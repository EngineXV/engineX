...
class CredentialSetupScreen(ModalScreen[bool | None]):
    ...
    def __init__(self, session: CredentialSetupSession) -> None:
        ...
        self._oauth_host_creds: set[int] = set()
        self._needs_oauth_host_key = False
        for i, cred in enumerate(self._missing):
            if cred.engine_oauth_supported and not cred.direct_api_key_supported:
                self._oauth_host_creds.add(i)
                self._needs_oauth_host_key = True
            ...
            with VerticalScroll(id="cred-scroll"):
                ...
                if self._needs_oauth_host_key:
                    ...
                    with Vertical(classes="cred-entry"):
                        ...
                        oauth_names = [
                            self._missing[i].credential_name for i in sorted(self._oauth_host_creds)
                        ]
                ...
                for i, cred in enumerate(self._missing):
                    if i in self._oauth_host_creds:
    ...
    def _save_credentials(self) -> None:
        ...
        if self._needs_oauth_host_key:
            ...
            if oauth_api_key:
                from framework.credentials.key_storage import save_oauth_host_api_key
            ...
            if oauth_api_key or os.environ.get("ENGINE_OAUTH_API_KEY"):
                self._sync_oauth_host_credentials()
    ...
    def _sync_oauth_host_credentials(self) -> int:
        ...
        synced = 0
        for i in sorted(self._oauth_host_creds):
