...
class ToolRegistry:
    ...
    _CREDENTIAL_DIR = Path("~/.engine/credentials/credentials").expanduser()
    ...
    def __init__(self):
        ...
        self._mcp_oauth_host_key_snapshot: str | None = None  # ENGINE_OAUTH_API_KEY value at MCP load time
    ...
    def load_mcp_config(self, config_path: Path) -> None:
        ...
        # Snapshot credential files and ENGINE_OAUTH_API_KEY so we can detect mid-session changes
        self._mcp_cred_snapshot = self._snapshot_credentials()
        self._mcp_oauth_host_key_snapshot = os.environ.get("ENGINE_OAUTH_API_KEY")
    ...
    def build_provider_index(self) -> None:
        ...
        try:
            ...
            logger.debug("engine_tools not available, skipping provider index")
            return

        self._provider_index.clear()
        for spec in CREDENTIAL_SPECS.values():
            provider = spec.engine_oauth_provider
            if provider:
    ...
    def resync_mcp_servers_if_needed(self) -> bool:
        ...
        current_oauth_host_key = os.environ.get("ENGINE_OAUTH_API_KEY")
        files_changed = current != self._mcp_cred_snapshot
        oauth_api_key_changed = current_oauth_host_key != self._mcp_oauth_host_key_snapshot

        if not files_changed and not oauth_api_key_changed:
            return False

        reason = (
            "Credential files and ENGINE_OAUTH_API_KEY changed"
            if files_changed and oauth_api_key_changed
            else "ENGINE_OAUTH_API_KEY changed"
            if oauth_api_key_changed
