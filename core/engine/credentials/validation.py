...
def _presync_engine_sync_tokens(credential_specs: dict, *, force: bool = False) -> None:
...
def validate_agent_credentials(
    ...
    empty_result = CredentialValidationResult(credentials=[], has_engine_sync_key=False)
    ...
    if os.environ.get("ENGINE_OAUTH_API_KEY"):
        _presync_engine_sync_tokens(CREDENTIAL_SPECS, force=force_refresh)
    ...
    has_engine_sync_key = bool(os.environ.get("ENGINE_OAUTH_API_KEY"))
    checked: set[str] = set()
    ...
    def _check_credential(
        ...
        status = CredentialStatus(
            ...
            oauth_not_connected=is_engine_sync_nc,
    ...
    validation_result = CredentialValidationResult(
        ...
        has_engine_sync_key=has_engine_sync_key,
    )
...
def _status_to_missing(c: CredentialStatus):
    ...
    from engine.credentials.setup import MissingCredential
