...
# Idempotent via ``window.__engine_vp_instrumented``.
_ENGINE_VP_PROBE_JS = """
(function () {
  if (window.__engine_vp_instrumented) return;
  window.__engine_vp_instrumented = true;
  function sample(kind) {
    try {
      console.info('[engine_vp]', JSON.stringify({
        kind: kind,
...
class BeelineBridge:
        ...
        try:
            async for raw in ws:
                ...
                if msg.get("type") == "cdp_event":
                    ...
                    # the [engine_vp] console probe as a structured
    ...
    def _handle_cdp_event(self, tab_id: int | None, method: str, params: dict) -> None:
        ...
        Runtime.consoleAPICalled with our ``[engine_vp]`` prefix is
        ...
        if method == "Runtime.consoleAPICalled":
            ...
            # Structured [engine_vp] viewport probe → viewport_event
            if first == "[engine_vp]" and isinstance(payload, str):
                try:
            ...
            if first == "[engine_attach_canary]" and isinstance(payload, str):
                try:
                    parsed = json.loads(payload)
                except Exception:
                    parsed = {"_raw": payload}
        ...
        # Our [engine_vp] probe's event listeners die with it. Reinstall
    ...
    def _schedule_reinject_probe(self, tab_id: int) -> None:
        """Fire-and-forget re-injection of _ENGINE_VP_PROBE_JS on the
        ...
        async def _do() -> None:
            try:
                ...
                # __engine_vp_instrumented — the probe's idempotency
                # guard works because nav cleared window state.
                await self._cdp(
                    tab_id,
                    "Runtime.evaluate",
                    {
                        "expression": _ENGINE_VP_PROBE_JS,
                        "returnByValue": True,
                        "awaitPromise": False,
                    },
                )
    ...
    async def cdp_attach(self, tab_id: int) -> dict:
        ...
        # diagnostic viewport-change probe ([engine_vp] console
        ...
        # [engine_vp] probe — install resize / visibility listeners on
        ...
        # handler. Idempotent via __engine_vp_instrumented.
        try:
            await self._cdp(
                tab_id,
                "Runtime.evaluate",
        ...
        # ``[engine_attach_canary]``. Zero canary entries after a
        # run means the extension forwarder is stale and the user
        # needs to reload the Engine extension in chrome://extensions.
        try:
            await self._cdp(
                tab_id,
                "Runtime.evaluate",
                {
                    "expression": (
                        "console.info('[engine_attach_canary]', "
    ...
    async def highlight_rect(
        ...
        js = f"""
        (function() {{
          // Remove any previous engine highlight (including its observer).
          var prev = document.getElementById('__engine_hl');
          if (prev) {{
            try {{ prev.__engineStop && prev.__engineStop(); }} catch(e) {{}}
            prev.remove();
          }}
          ...
          var obs = new MutationObserver(function() {{
            if (stopped) return;
            if (!document.getElementById('__engine_hl')) {{
    ...
    async def clear_highlight(self, tab_id: int) -> None:
        ...
        try:
            await self.evaluate(
                tab_id,
                """
                var el = document.getElementById('__engine_hl');
                if (el) el.remove();
            """,
            )
        except Exception:
