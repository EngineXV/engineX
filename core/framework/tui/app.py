...
class EngineTUI(App):
    ...
    async def on_mount(self) -> None:
        ...
        self.title = "Engine TUI Dashboard"
    ...
    async def _load_judge_and_queen(self, storage_path) -> None:
        """Queen/judge monitoring removed in minimal Engine runtime"""
        return
    ...
    async def _do_finish_agent_load(self, runner) -> None:
        """Worker wrapper for _finish_agent_load"""
        await self._finish_agent_load(runner)
    ...
    def _show_credential_setup(
        ...
        if not session.missing:
            ...
            if "not connected" in error_msg or "Engine" in error_msg:
                self.notify(
                    "ENGINE_OAUTH_API_KEY is set but OAuth integrations "
                    "are not connected. Visit engine.localhost "
                ...
                self.notify(
                    ...
                    "detected. Run 'engine setup-credentials' "
                    "from the terminal.",
                    severity="error",
                    timeout=10,
                )
    ...
    def _show_build_edit_message(self) -> None:
        ...
        self.notify(
            "To build or edit agents, use 'engine build' from the terminal "
            "or run Claude Code with the /engine skill.",
    ...
    async def _do_escalate_to_coder(
        ...
        """Engine Coder escalation removed in minimal runtime"""
        del reason, context, node_id
        self.notify("Engine Coder is not included in this minimal runtime", severity="warning")

    def _build_escalation_input(self, reason: str, context: str, worker_path: str) -> str:
        """Compose the user_request string for engine_coder"""
        parts = []
        if worker_path:
            parts.append(
    ...
    async def _return_from_escalation(self, summary: str = "") -> None:
        ...
        def _show_return():
            if self.chat_repl:
                self.chat_repl._write_history("[bold cyan]Returned from Engine Coder.[/bold cyan]")
                if summary:
    ...
    def action_pause_execution(self) -> None:
        """Immediately pause execution by cancelling all running tasks"""
    ...
    def action_escalate_to_coder(self) -> None:
        """Escalate to Engine Coder (bound to Ctrl+E)"""
        if self.runtime is None:
    ...
    async def action_return_from_coder(self) -> None:
        """Return from Engine Coder to worker agent (Ctrl+E toggles)"""
