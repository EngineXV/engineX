...
# ---------------------------------------------------------------------------
# ENGINE_DEBUG_EVENTS — write every published event to a JSONL file.
#
# Set the env var to any truthy value to enable:
#   ENGINE_DEBUG_EVENTS=1          → writes to ~/.engine/event_logs/<ts>.jsonl
#   ENGINE_DEBUG_EVENTS=/tmp/ev    → writes to that exact directory
#
...
# ---------------------------------------------------------------------------
_DEBUG_EVENTS_RAW = os.environ.get("ENGINE_DEBUG_EVENTS", "").strip()
...
def _open_event_log() -> IO[str] | None:
    ...
    if raw.lower() in ("1", "true", "full"):
        log_dir = Path.home() / ".engine" / "event_logs"
    else:
        log_dir = Path(raw)
...
class EventBus:
    ...
    async def publish(self, event: AgentEvent) -> None:
        ...
        # Write event to JSONL file (gated by ENGINE_DEBUG_EVENTS env var)
    ...
    async def emit_escalation_requested(
        ...
        """Emit escalation requested event (agent wants engine_coder)"""
        await self.publish(
            AgentEvent(
