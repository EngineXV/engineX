...
Run via:
    ...
    uv run python terminal_tools_server.py --stdio
"""
...
async def _lifespan(_server: FastMCP) -> AsyncIterator[dict]:
    ...
    """
    parent_pid_env = os.getenv("ENGINE_DESKTOP_PARENT_PID")
    if parent_pid_env:
        try:
            ...
            logger.warning("Invalid ENGINE_DESKTOP_PARENT_PID=%r", parent_pid_env)
