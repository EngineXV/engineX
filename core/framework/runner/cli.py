...
def register_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    run_parser.add_argument(
        ...
        "-r",
        action="store_true",
        help="Print time distribution report after the run",
    )
    ...
    shell_parser.add_argument(
        ...
        "-r",
        action="store_true",
        help="Print time distribution report after each turn",
    )
...
def _run_time_report(agent_name: str) -> None:
    """Run the time distribution report for the given agent (table format)"""
    import subprocess

    project_root = Path(__file__).resolve().parent.parent.parent.parent
    script = project_root / "report_time_distribution.py"
    if not script.exists():
        print(
            f"Report script not found at {script}. Run from project root.",
            file=sys.stderr,
        )
        return
    try:
        subprocess.run(
            [sys.executable, str(script), "--agent", agent_name, "--table"],
            check=False,
        )
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "tui", False):
        try:
            from framework.tui.app import EngineTUI
    ...
    if getattr(args, "report", False) and not args.quiet:
        agent_name = Path(args.agent_path).name
        _run_time_report(agent_name)
...
def _format_natural_language_to_json(
    ...
    """Use Haiku to convert natural language input to JSON based on"""
    import os
...
def cmd_shell(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if getattr(args, "report", False):
            _run_time_report(Path(agent_path).name)
...
def _launch_agent_tui(
    ...
    async def run_with_tui():
        ...
        app = EngineTUI(runner._agent_runtime)
        try:
            await app.run_async()
        except Exception as e:
            import traceback
