"""Adapter to integrate the new CredentialStore"""
...
class CredentialStoreAdapter:
    """Adapter that makes CredentialStore compatible"""
    ...
    def activate_local_account(self, credential_id: str, alias: str) -> bool:
        """Inject a named local account's API key into the environment"""
        import os
    ...
    def default(
        ...
        # --- Engine sync branch ---
        # Note: we don't use CredentialStore.with_engine_sync() here because it
        # only wraps EncryptedFileStorage.  We need CompositeStorage (encrypted
        # + env var fallback) so non-Engine credentials like brave_search still
        ...
        if engine_oauth_api_key:
            try:
                ...
                # Engine components
                client = EngineCredentialClient(
                    EngineClientConfig(
                        base_url=os.environ.get("ENGINE_OAUTH_API_URL", "https://api.localhost"),
                    )
                )
                ...
                # Initial sync: populate local cache from Engine
                try:
                    synced = provider.sync_all(store)
                    log.info("Engine credential sync complete: %d credentials synced", synced)
                except Exception as e:
                    log.warning("Engine initial sync failed (will retry on access): %s", e)

                return cls(store=store, specs=specs)

            except Exception as e:
                log.warning(
                    "Engine credential sync unavailable, falling back to default storage: %s", e
                )

        # --- Default branch (no ENGINE_OAUTH_API_KEY or Engine setup failed) ---
        try:
            encrypted = EncryptedFileStorage()
            env = EnvVarStorage(env_mapping)
