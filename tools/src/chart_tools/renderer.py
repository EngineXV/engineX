...
async def _render_in_page(
    ...
    *,
    ...
    if kind == "echarts":
        ...
        # Register the Engine theme on the page, then init with that
        ...
        result = await page.evaluate(
            """async ({option, themeJson}) => {
              try {
                echarts.registerTheme('engine', JSON.parse(themeJson));
                const el = document.getElementById('chart');
                const chart = echarts.init(el, 'engine', {renderer: 'svg'});
