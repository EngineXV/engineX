"""Tests for the engine CLI entry point and path auto-configuration."""

import shutil
import subprocess
import sys
from pathlib import Path

import pytest

from engine.cli import _configure_paths


@pytest.fixture
def project_root():
    """Return the project root directory."""
    return Path(__file__).resolve().parent.parent.parent


class TestConfigurePaths:
    """Test _configure_paths auto-discovers exports/ and core/."""

    def test_adds_exports_to_sys_path(self, project_root):
        exports_dir = project_root / "exports"
        if not exports_dir.is_dir():
            pytest.skip("exports/ directory does not exist in this environment")

        exports_str = str(exports_dir)
        # Remove if already present to test fresh addition
        original_path = sys.path.copy()
        sys.path = [p for p in sys.path if p != exports_str]

        try:
            _configure_paths()
            assert exports_str in sys.path
        finally:
            sys.path = original_path

    def test_adds_core_to_sys_path(self, project_root):
        core_dir = project_root / "core"
        core_str = str(core_dir)
        original_path = sys.path.copy()
        sys.path = [p for p in sys.path if p != core_str]

        try:
            _configure_paths()
            assert core_str in sys.path
        finally:
            sys.path = original_path

    def test_does_not_duplicate_paths(self):
        _configure_paths()
        # Call twice — should not create duplicates
        before = sys.path.copy()
        _configure_paths()
        assert sys.path == before

    def test_handles_missing_exports_gracefully(self):
        """If exports/ doesn't exist, _configure_paths should not crash."""
        _configure_paths()


class TestEngineModule:
    """Test ``python -m engine`` invocation."""

    def test_module_help(self, project_root):
        """Verify ``python -m engine --help`` prints usage."""
        result = subprocess.run(
            [sys.executable, "-m", "engine", "--help"],
            capture_output=True,
            text=True,
            cwd=str(project_root / "core"),
        )
        assert result.returncode == 0
        assert "engine" in result.stdout.lower() or "goal" in result.stdout.lower()

    def test_module_list_subcommand(self, project_root):
        """Verify ``python -m engine list --help`` registers the subcommand."""
        result = subprocess.run(
            [sys.executable, "-m", "engine", "list", "--help"],
            capture_output=True,
            text=True,
            cwd=str(project_root / "core"),
        )
        assert result.returncode == 0
        assert "agents" in result.stdout.lower() or "directory" in result.stdout.lower()

    def test_module_config_validate_help(self, project_root):
        result = subprocess.run(
            [sys.executable, "-m", "engine", "config", "validate", "--help"],
            capture_output=True,
            text=True,
            cwd=str(project_root / "core"),
        )
        assert result.returncode == 0
        assert "configuration" in result.stdout.lower()


class TestEngineEntryPoint:
    """Test the ``engine`` console_scripts entry point.

    These tests verify the actual ``engine`` command installed by
    ``pip install -e core/``. If the entry point is not installed,
    the tests are skipped gracefully.
    """

    @pytest.fixture(autouse=True)
    def _require_engine(self):
        if shutil.which("engine") is None:
            pytest.skip("'engine' entry point not installed (run: pip install -e core/)")

    def test_engine_help(self):
        """Verify ``engine --help`` exits 0 and prints usage."""
        result = subprocess.run(
            ["engine", "--help"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
        assert "run" in result.stdout.lower()
        assert "validate" in result.stdout.lower()

    def test_engine_list_help(self):
        """Verify ``engine list --help`` exits 0."""
        result = subprocess.run(
            ["engine", "list", "--help"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0

    def test_engine_run_missing_agent(self):
        """Verify ``engine run`` with a non-existent agent prints an error."""
        result = subprocess.run(
            ["engine", "run", "nonexistent_agent_xyz"],
            capture_output=True,
            text=True,
        )
        assert result.returncode != 0
