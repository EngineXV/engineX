...
class PipelineRunner:
    """Runs middleware stages on each AgentRuntime trigger."""
    ...
    def add_stage(self, stage: PipelineStage) -> None:
        """Register a stage after construction."""
    ...
    async def run(self, ctx: PipelineContext) -> PipelineContext:
        ...
        for stage in self._stages:
            ...
            if result.action == "transform":
                ...
                logger.info("[pipeline] %s passed (%.1fms)", stage_name, elapsed_ms)
    ...
    async def run_post(self, ctx: PipelineContext, result: Any) -> Any:
        """Run post_process hooks without failing a successful execution."""
        current = result
        for stage in self._stages:
            try:
