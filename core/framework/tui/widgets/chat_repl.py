...
class ChatRepl(Vertical):
  ...
  [bold]/coder[/bold] [reason]             - Escalate to Engine Coder for code changes
  [bold]/back[/bold] [summary]             - Return from Engine Coder to worker agent
...
""")
    ...
    def attach_pdf(self, path: Path) -> None:
        ...
        # Copy to ~/.engine/assets/, deduplicating like a normal filesystem:
        # resume.pdf → resume(1).pdf → resume(2).pdf
        assets_dir = Path.home() / ".engine" / "assets"
    ...
    def _get_session_label(self, state: dict) -> str:
        """Extract the first user message from input_data as a human-readable"""
    ...
    async def _cmd_coder(self, reason: str = "") -> None:
        """User-initiated escalation to Engine Coder"""
        app = self.app
        ...
        self._write_history("[bold cyan]Escalating to Engine Coder...[/bold cyan]")
    ...
    async def _cmd_back(self, summary: str = "") -> None:
        """Return from Engine Coder to the worker agent"""
        app = self.app
    ...
    def handle_escalation_requested(self, data: dict) -> None:
        ...
        self._write_history("[bold yellow]Agent is escalating to Engine Coder[/bold yellow]")
        if reason:
    ...
    def handle_internal_output(self, node_id: str, content: str) -> None:
        """Buffer output from non-client-facing nodes. Only display if logs"""
