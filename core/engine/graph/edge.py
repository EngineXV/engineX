...
class EdgeSpec(BaseModel):
    ...
    def _evaluate_condition(
        ...
        try:
            ...
            logger.warning(f"      WARN: Condition evaluation failed: {self.condition_expr}")
...
{{"proceed": true/false, "reasoning": "brief explanation"}}"""
        ...
        try:
            ...
            logger.warning(f"      WARN: LLM routing failed, defaulting to on_success: {e}")
            return source_success
...
class AsyncEntryPointSpec(BaseModel):
    ...
    def get_isolation_level(self):
        ...
        from engine.runtime.execution_stream import IsolationLevel
...
class GraphSpec(BaseModel):
    ...
    def _resolve_max_tokens(cls, values: Any) -> Any:
        """Resolve max_tokens from the global config store"""
        if isinstance(values, dict) and values.get("max_tokens") is None:
            from engine.config import get_max_tokens
