"""Dedicated file-based storage for bootstrap credentials"""

from __future__ import annotations

import logging
import os
import stat
from pathlib import Path

logger = logging.getLogger(__name__)

CREDENTIAL_KEY_PATH = Path.home() / ".engine" / "secrets" / "credential_key"
CREDENTIAL_KEY_ENV_VAR = "ENGINE_CREDENTIAL_KEY"
ENGINE_OAUTH_CREDENTIAL_ID = "engine_oauth_api_key"
ENGINE_OAUTH_ENV_VAR = "ENGINE_OAUTH_API_KEY"


# ---------------------------------------------------------------------------
# ENGINE_CREDENTIAL_KEY
# ---------------------------------------------------------------------------


def load_credential_key() -> str | None:
    """Load ENGINE_CREDENTIAL_KEY with priority: env > file > shell config"""
    # 1. Already in environment (set by parent process, CI, Windows Registry, etc.)
    key = os.environ.get(CREDENTIAL_KEY_ENV_VAR)
    if key:
        return key

    # 2. Dedicated secrets file
    key = _read_credential_key_file()
    if key:
        os.environ[CREDENTIAL_KEY_ENV_VAR] = key
        return key

    # 3. Shell config fallback (backward compat for old installs)
    key = _read_from_shell_config(CREDENTIAL_KEY_ENV_VAR)
    if key:
        os.environ[CREDENTIAL_KEY_ENV_VAR] = key
        return key

    return None


def save_credential_key(key: str) -> Path:
    """Save ENGINE_CREDENTIAL_KEY to ``~/.engine/secrets/credential_key``"""
    path = CREDENTIAL_KEY_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    # Restrict the secrets directory itself
    path.parent.chmod(stat.S_IRWXU)  # 0o700

    path.write_text(key)
    path.chmod(stat.S_IRUSR | stat.S_IWUSR)  # 0o600

    os.environ[CREDENTIAL_KEY_ENV_VAR] = key
    return path


def generate_and_save_credential_key() -> str:
    """Generate a new Fernet key and persist it to"""
    from cryptography.fernet import Fernet

    key = Fernet.generate_key().decode()
    save_credential_key(key)
    return key


# ---------------------------------------------------------------------------
# ENGINE_OAUTH_API_KEY
# ---------------------------------------------------------------------------


def load_oauth_host_api_key() -> str | None:
    """env > encrypted store > shell config"""
    # 1. Already in environment
    key = os.environ.get(ENGINE_OAUTH_ENV_VAR)
    if key:
        return key

    # 2. Encrypted credential store
    key = _read_oauth_host_from_encrypted_store()
    if key:
        os.environ[ENGINE_OAUTH_ENV_VAR] = key
        return key

    # 3. Shell config fallback (backward compat)
    key = _read_from_shell_config(ENGINE_OAUTH_ENV_VAR)
    if key:
        os.environ[ENGINE_OAUTH_ENV_VAR] = key
        return key

    return None


def save_oauth_host_api_key(key: str) -> None:
    """Save ENGINE_OAUTH_API_KEY to the encrypted credential store"""
    from pydantic import SecretStr

    from .models import CredentialKey, CredentialObject
    from .storage import EncryptedFileStorage

    storage = EncryptedFileStorage()
    cred = CredentialObject(
        id=ENGINE_OAUTH_CREDENTIAL_ID,
        keys={"api_key": CredentialKey(name="api_key", value=SecretStr(key))},
    )
    storage.save(cred)
    os.environ[ENGINE_OAUTH_ENV_VAR] = key


def delete_oauth_host_api_key() -> None:
    """Remove ENGINE_OAUTH_API_KEY from the encrypted store and"""
    try:
        from .storage import EncryptedFileStorage

        storage = EncryptedFileStorage()
        storage.delete(ENGINE_OAUTH_CREDENTIAL_ID)
    except Exception:
        logger.debug("Could not delete %s from encrypted store", ENGINE_OAUTH_CREDENTIAL_ID)

    os.environ.pop(ENGINE_OAUTH_ENV_VAR, None)


# ---------------------------------------------------------------------------
# Internal helpers
# ---------------------------------------------------------------------------


def _read_credential_key_file() -> str | None:
    """Read the credential key from ``~/.engine/secrets/credential_key``"""
    try:
        if CREDENTIAL_KEY_PATH.is_file():
            value = CREDENTIAL_KEY_PATH.read_text(encoding="utf-8").strip()
            if value:
                return value
    except Exception:
        logger.debug("Could not read %s", CREDENTIAL_KEY_PATH)
    return None


def _read_from_shell_config(env_var: str) -> str | None:
    """Fallback: read an env var from ~/.zshrc or ~/.bashrc"""
    try:
        from engine_tools.credentials.shell_config import check_env_var_in_shell_config

        found, value = check_env_var_in_shell_config(env_var)
        if found and value:
            return value
    except ImportError:
        pass
    return None


def _read_oauth_host_from_encrypted_store() -> str | None:
    """Try to load ENGINE_OAUTH_API_KEY from the encrypted credential store"""
    if not os.environ.get(CREDENTIAL_KEY_ENV_VAR):
        return None
    try:
        from .storage import EncryptedFileStorage

        storage = EncryptedFileStorage()
        cred = storage.load(ENGINE_OAUTH_CREDENTIAL_ID)
        if cred:
            return cred.get_key("api_key")
    except Exception:
        logger.debug("Could not load %s from encrypted store", ENGINE_OAUTH_CREDENTIAL_ID)
    return None
