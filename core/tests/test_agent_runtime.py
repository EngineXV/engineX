...
from engine.runtime.agent_runtime import AgentRuntime, create_agent_runtime
...
def sample_graph():
    ...
    nodes = [
        NodeSpec(
            id="process-job",
            name="Process Job",
            description="Process incoming job",
    ...
    return GraphSpec(
        ...
        version="1.0.0",
        entry_node="process-job",
        entry_points={"start": "process-job"},
...
class TestEventBus:
    ...
    async def test_publish_subscribe(self):
        ...
        assert received_events[0].stream_id == "api"
    ...
    async def test_stream_filter(self):
        ...
        # Publish to api stream (should be received)
        await bus.publish(
            AgentEvent(
                type=EventType.EXECUTION_STARTED,
                stream_id="api",
        ...
        # Publish to manual stream (should NOT be received)
        await bus.publish(
            AgentEvent(
                type=EventType.EXECUTION_STARTED,
                stream_id="manual",
...
class TestOutcomeAggregator:
    ...
    def test_record_decision(self, sample_goal):
        ...
        from engine.schemas.decision import Decision, DecisionType

        decision = Decision(
            id="dec-1",
            node_id="process-job",
            intent="Process incoming job",
        ...
        aggregator.record_decision("api", "exec-1", decision)
...
class TestAgentRuntime:
    ...
    async def test_start_stop_lifecycle(self, sample_graph, sample_goal, temp_storage):
        ...
        assert "manual" in runtime._streams
    ...
    async def test_trigger_requires_running(self, sample_graph, sample_goal, temp_storage):
        ...
        with pytest.raises(RuntimeError, match="not running"):
            await runtime.trigger("manual", {"test": "data"})
...
class TestGraphSpecValidation:
    ...
    def test_get_async_entry_point(self, sample_graph):
        ...
        ep = sample_graph.get_async_entry_point("manual")
        assert ep is not None
        assert ep.id == "manual"
        assert ep.entry_node == "process-job"
...
class TestTimerEntryPoints:
    ...
    async def test_cron_immediate_fires_first(self, sample_graph, sample_goal, temp_storage):
        """Test that run_immediately=True"""
        runtime = AgentRuntime(
            graph=sample_graph,
            goal=sample_goal,
            storage_path=temp_storage,
...
class TestCancelAllTasks:
    ...
    async def test_cancel_all_tasks_async_cancels_running_task(
        ...
        """Test that cancel_all_tasks_async cancels a running task and"""
        runtime = AgentRuntime(
            graph=sample_graph,
            goal=sample_goal,
            storage_path=temp_storage,
        ...
        try:
            ...
            stream = runtime._streams["manual"]
