...
def register_file_tools(
    ...
    *,
    ...
    @mcp.tool()
    def write_file(path: str, content: str = "", data: str = "") -> str:
        """Create or overwrite a file with the given content"""
        text = content if content else data
        if not text:
            return "Error: Must provide either content or data"
        ...
        try:
            ...
            return f"{action} {path} ({len(text):,} bytes, {line_count} lines)"
        except Exception as e:
